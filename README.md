...
- Azure Kinect K4A wrapper 기반 CUDA body tracking (lite 모델 옵션)
...
- `processingModes`: CUDA만 사용합니다. 기본 `["Cuda"]`이며 DirectML/Cpu fallback은 현장 운용에서 비활성화되어 있습니다. CUDA 런타임 DLL이 없으면 skeleton/depth fallback으로 내려가지 않고 에러로 중단합니다.
...
DLL이 없으면 DSCC는 DirectML/Cpu로 폴백하지 않고 시작을 중단합니다. 4대 동시 운용은 CUDA + lite 모델 조합을 기준으로 검증합니다.
